...
logger = get_logger("Maizone.diary.service")
...
class DiaryService:
    ...
    async def _generate_with_custom_model(self, prompt: str) -> Tuple[bool, str]:
        try:
            ...
            client = AsyncOpenAI(
                base_url=self.get_config("diary.model.api_url", "https://api.siliconflow.cn/v1"),
                api_key=api_key,
            )
            api_timeout = self.get_config("diary.model.api_timeout", 300)
            if not (1 <= api_timeout <= 6000):
                api_timeout = 300
            completion = await client.chat.completions.create(
                model=self.get_config("diary.model.model_name", "Pro/deepseek-ai/DeepSeek-V3"),
                messages=[{"role": "user", "content": prompt}],
                temperature=self.get_config("diary.model.temperature", 0.7),
                timeout=api_timeout,
            )
    ...
    async def generate_diary_from_messages(
        ...
        try:
            ...
            min_wc = self.get_config("diary.min_word_count", 250)
            max_wc = self.get_config("diary.max_word_count", 350)
            ...
            style = self.get_config("diary.style", "diary")
            if style == "custom":
                template = self.get_config("diary.custom_prompt", "") or ""
                context = {
                    "date": date,
                    "timeline": timeline,
...
日记内容:"""
            ...
            use_custom_model = self.get_config("diary.model.use_custom_model", False)
            if use_custom_model:
            ...
            max_length = self.get_config("diary.max_word_count", 350)
    ...
    async def publish_to_qzone(self, date: str, diary_content: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                await get_napcat_config_and_renew(_get_config)
            except Exception as e:
                logger.warning(f"发布前刷新cookie失败: {e}，尝试继续发布")

            # 使用 Maizone 的统一 Cookie 管理和 QZone API
            ...
            if not qzone:
                ...
                if diary_data:
                    ...
                    diary_data["error_message"] = "原因:创建QzoneAPI失败,cookie不存在"
                    await self.storage.save_diary(diary_data)
                return False
            tid = await qzone.publish_emotion(diary_content, [])
